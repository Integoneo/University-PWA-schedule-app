...
<template>
  <div class="h-[100dvh] w-full bg-slate-950 flex flex-col justify-between p-5 sm:p-6 overflow-hidden relative">
    ...
    <div class="flex flex-col items-center mt-6 sm:mt-12 relative z-10 text-center">
      ...
      <div v-if="isModernBrowser" class="w-16 h-16 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-500 p-0.5 shadow-2xl shadow-indigo-500/20 mb-4 sm:mb-6">
        <div class="w-full h-full bg-slate-950 rounded-[14px] flex items-center justify-center">
          <svg class="w-8 h-8 text-indigo-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
        </div>
      </div>
      
      <!-- Добавили pb-1, чтобы градиент не обрезал хвосты у букв "g" и "p" -->
      <h1 class="text-3xl sm:text-4xl font-black tracking-tight bg-gradient-to-br from-white to-slate-400 bg-clip-text text-transparent mb-1 sm:mb-2 pb-1">
        Kosyga.Space
      </h1>
      <span class="text-[11px] sm:text-xs font-bold text-indigo-400 uppercase tracking-widest">
    ...
    <div class="flex flex-col gap-3 sm:gap-5 relative z-10 mb-4 sm:mb-8">
      ...
      <div class="bg-slate-900/40 border border-slate-800 rounded-3xl p-4 sm:p-5 shadow-lg">
        <h3 class="text-white font-bold text-base sm:text-lg mb-1.5 sm:mb-2">Привет! 👋</h3>
    ...
    <div v-if="isModernBrowser" class="bg-amber-500/10 border border-amber-500/20 rounded-3xl p-3.5 sm:p-4 flex gap-2.5 sm:gap-3">
        <span class="text-amber-500 text-base sm:text-lg shrink-0">⚠️</span>
        <p class="text-[10px] sm:text-[11px] text-amber-500/80 leading-snug">
    ...
    <div class="mb-2 sm:mb-4 relative z-10 w-full">
      <button 
        ...
        class="w-full py-3.5 sm:py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-2xl transition-transform active:scale-95 shadow-xl shadow-indigo-900/50 flex items-center justify-center gap-2 text-sm sm:text-base"
      >
        Найти свою группу
